쿠폰 템플릿 배포\*를 위해 필요한 데이터(이름, 심볼)을 전송하고, 응답값으로 참조 토큰을 받습니다. 

- 쿠폰 템플릿의 owner 는 wallet_id 필드의 주소로 지정됩니다.
- 쿠폰 템플릿의 default_data 는 빈문자열("") 로 지정됩니다.

\* 쿠폰 템플릿 배포: 쿠폰 발행을 위해 쿠폰 정보를 담아 템플릿을 배포 (블록체인 상 토큰 발행 정보를 담은 컨트랙트를 배포하는 단계, 배포 후 수정 불가)
